# @version 0.3.7
"""
@title Mushroom Token Vault
@license MIT
@author mushroom.finance
@notice
    Mushroom Token Vault. Holds an underlying token, and allows users to interact
    with the Mushroom ecosystem through Strategies connected to the Vault.
    ...
    https://github.com/mushroom-finance/mushroom-devdocs/blob/master/docs/developers/v2/SPECIFICATION.md
"""
...
def initialize(
    ...
    @dev
        If `nameOverride` is not specified, the name will be 'mushroom'
    ...
    if nameOverride == "":
        self.name = concat(DetailedERC20(token).symbol(), " mVault")
    else:
        self.name = nameOverride
    if symbolOverride == "":
        self.symbol = concat("mv", DetailedERC20(token).symbol())
    else:
...
def apiVersion() -> String[28]:
    ...
    @notice
        ...
        can use this version number to compare with Mushroom's GitHub and
...
def domain_separator() -> bytes32:
    return keccak256(
        concat(
            DOMAIN_TYPE_HASH,
            keccak256(convert("Mushroom Vault", Bytes[14])),
