...
contract VyperDeployer {
     ...
     */
    function deploy(string memory path, string memory fileName) public returns (address) {
     ...
     * @return deployedAddress The address that the contract was deployed to.
     */
    function deploy(string memory path, string memory fileName, bytes memory args) public returns (address) {
        ///@notice create a list of strings with the commands necessary to compile Vyper contracts
        string[] memory cmds = new string[](2);
        cmds[0] = "vyper";
        cmds[1] = string.concat(path, fileName, ".vy");
